...
Electron 기반 데스크톱 Todo 앱. JOB(프로젝트) 단위로 Task를 관리하며 **보드 / 목록 / 캘린더** 세 가지 보기를 제공합니다.

## 주요 기능

### 3가지 보기 모드
- **보드** — JOB별 칸반 컬럼 (상태/우선순위 필터, 카드 직접 상태·우선순위 변경)
...
### 그 외
- **Task 속성**: 제목, JOB, 상태(대기/진행/완료), 우선순위(높음/보통/낮음), 시작일·시간, 마감일, 종료일·시간, 메모
- **시작 시간 알림** — 시스템 알림, 분 단위 사전 알림 시간 설정
- **확대/축소** — `Ctrl + 마우스휠` (본문 영역만, 3단계)
- **온보딩 투어** — 첫 실행 시 자동, 우상단 `?` 버튼으로 다시 보기
